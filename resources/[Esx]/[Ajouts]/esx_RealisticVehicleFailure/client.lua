...
RegisterNetEvent('esx:seetjobmd')
AddEventHandler('esx:seetjobmd', function(job)
    PlayerData.job = job
end)
...
AddEventHandler('iens:repair2', function()
    ...
    if GetVehicleEngineHealth(vehicle) <= 101 then
        SetVehicleUndriveable(vehicle, false)
        SetVehicleEngineHealth(vehicle, cfg.cascadingFailureThreshold + 5)
        SetVehiclePetrolTankHealth(vehicle, 750.0)
        healthEngineLast = cfg.cascadingFailureThreshold + 5
        ...
        SetVehicleEngineOn(vehicle, true, false)
        SetVehicleOilLevel(vehicle, (GetVehicleOilLevel(vehicle) / 3) - 0.5)
        notification("🔧 ~g~Tu à fait une petitre réparation, mais ton véhicule ne vas pas tenir longtemps !")
    else
        notification("🔧 ~g~Tu n'a pas besoin de réparer ton véhicule !")
    end
end)
                    ...
                    if cfg.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
                        ...
                        if speed >= 1.0 then
                            ...
                            if accelerator > 127 then
                                ...
                                local acc = fscale(accelerator, 127.0, 254.0, 0.1, 1.0,
                            ...
                            if brake > 127 then
                                ...
                                brk = fscale(brake, 127.0, 254.0, 0.01, fBrakeForce,
                        ...
                        elseif speed <= -1.0 then
                            ...
                            if brake > 127 then
                                ...
                                local rev = fscale(brake, 127.0, 254.0, 0.1, 1.0,
                                                10.0 - (cfg.sundayDriverAcceleratorCurve * 2.0))
                                factor = factor * rev
                            end
                            if accelerator > 127 then
                                ...
                                brk = fscale(accelerator, 127.0, 254.0, 0.01, fBrakeForce,
                                          10.0 - (cfg.sundayDriverBrakeCurve * 2.0))
                            end
                        else
        ...
        if isPedDrivingAVehicle() then
            ...
            if pedInSameVehicleLast == true then
                ...
                if healthEngineCurrent ~= 1000.0 or healthBodyCurrent ~= 1000.0 or healthPetrolTankCurrent ~= 1000.0 then
                    ...
                    if healthEngineNew > (cfg.cascadingFailureThreshold + 5) and healthEngineNew <
                        cfg.degradingFailureThreshold then
                ...
                local newFDeformationDamageMult = fDeformationDamageMult ^
                                                      cfg.deformationExponent -- Pull the handling file value closer to 1
                    
                if cfg.deformationMultiplier ~= -1 then
                    SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fDeformationDamageMult',
                        newFDeformationDamageMult * cfg.deformationMultiplier)
                end -- Multiply by our factor
                if cfg.weaponsDamageMultiplier ~= -1 then
                    SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fWeaponDamageMult',
                ...
                local newFCollisionDamageMultiplier = fCollisionDamageMult ^
                                                          cfg.collisionDamageExponent -- Pull the handling file value closer to 1
                ...
                -- Modify it by pulling all number a towards 1.0
                local newFEngineDamageMult = fEngineDamageMult ^
                                                 cfg.engineDamageExponent -- Pull the handling file value closer to 1
            ...
            if pedInSameVehicleLast == true then
                ...
                if cfg.deformationMultiplier ~= -1 then
                    SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fDeformationDamageMult',
                ...
                if cfg.weaponsDamageMultiplier ~= -1 then
                    SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fWeaponDamageMult',
...
--     while true do
--         Citizen.Wait(0)
--         if IsControlJustPressed(0, 307) then -- right
--             if IsPedInAnyVehicle(GetPlayerPed(-1), true) then
--                 TriggerEvent('kyo:Indicator', 'right')
--             end
--         end

--         if IsControlJustPressed(0, 308) then -- left
--             if IsPedInAnyVehicle(GetPlayerPed(-1), true) then
--                 TriggerEvent('kyo:Indicator', 'left')
--             end
--         end
--     end
-- end)

-- AddEventHandler('kyo:Indicator', function(dir)
--     Citizen.CreateThread(function()
--         local Ped = GetPlayerPed(-1)
...
--                     TriggerServerEvent('kyo:IndicatorL', IndicatorL)
--                 elseif dir == 'right' then
--                     IndicatorR = not IndicatorR
--                     TriggerServerEvent('kyo:IndicatorR', IndicatorR)
--                 end
--             end
--         end
--     end)
...
-- RegisterNetEvent('kyo:updateIndicators')
-- AddEventHandler('kyo:updateIndicators', function(PID, dir, Toggle)

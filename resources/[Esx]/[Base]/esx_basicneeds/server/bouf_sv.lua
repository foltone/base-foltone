...
ESX.RegisterUsableItem('iceteagreen', function(source)
	local xPlayer = ESX.GetPlayerFromId(source)

	xPlayer.removeInventoryItem('iceteagreen', 1)

	TriggerClientEvent('esx_status:add', source, 'thirst', 300000)
	TriggerClientEvent('esx_basicneeds:onDrink', source)
	--TriggerClientEvent('esx:showNotification', source, _U('used_iceteagreen'))
end)
	...
	if args[1] then
		...
		if playerId then
			if GetPlayerName(playerId) then
				...
				TriggerClientEvent('chat:addMessage', source, { args = { '^2FiveLife', 'Vous avez bien était heal' } })
			else
				TriggerClientEvent('chat:addMessage', source, { args = { '^2FiveLife', 'Joueur déconnectée' } })
			end
		else
			TriggerClientEvent('chat:addMessage', source, { args = { '^2FiveLife', 'ID Incorrect' } })
		end
	else
		TriggerClientEvent('esx_basicneeds:healPlayer', source)
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^2FiveLife', 'Vous n\'avez pas les permissions' } })
end, {help = '', params = {{name = 'ID', help = ''}}})

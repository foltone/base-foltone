...
RMenu.Add('boutique', 'caissesmenu', RageUI.CreateSubMenu(RMenu:Get('boutique', 'main'), "Caisses ", "Menu d'caisses "))
...
function openboutique()
    if boutiquemgl then
		...
		RageUI.IsVisible(RMenu:Get('boutique', 'vehiclemenu'), true, true, true, function()
			for k, itemv in pairs(item_vehicule) do
				...
				RageUI.ButtonWithStyle(itemv.name, nil, { RightLabel = "~r~"..tostring(itemv.point).." ~b~"..moneypoints },true, function(h, a, s)
					if a then 
						--print(itemv.img)
						RenderSprite("RageUIf", itemv.img, 0, 565, 430, 200, 100)
					end
					if s then
		...
		RageUI.IsVisible(RMenu:Get('boutique', 'armesmenu'), true, true, true, function()
			for k, itemar in pairs(item_arme) do
                RageUI.ButtonWithStyle(itemar.name, nil, {RightLabel = "~r~"..tostring(itemar.point).." ~b~"..moneypoints}, true, function(h, a, s)
					if a then 
						--print(itemar.img)
						RenderSprite("RageUIf", itemar.img, 0, 520, 430, 200, 100)
					end
					if s then
		...
		RageUI.IsVisible(RMenu:Get('boutique', 'moneymenu'), true, true, true, function()
			for k, itemmoy in pairs(item_money) do
                RageUI.ButtonWithStyle(itemmoy.name, nil, {RightLabel = "~r~"..tostring(itemmoy.point).." ~b~"..moneypoints}, true, function(h, a, s)
					if a then 
						--print(itemmoy.img)
						RenderSprite("RageUIf", itemmoy.img, 0, 300, 430, 200, 100)
					end
					if s then
		...
		RageUI.IsVisible(RMenu:Get('boutique', 'caissesmenu'), true, true, true, function()
			for k, itemcais in pairs(item_caisses) do
                RageUI.ButtonWithStyle(itemcais.name, nil, {RightLabel = "~r~"..tostring(itemcais.point).." ~b~"..moneypoints}, true, function(h, a, s)
					if a then 
						--print(itemcais.img)
						RenderSprite("RageUIf", itemcais.img, 0, 300, 430, 200, 100)
					end
					if s then

						curentvehicle_name = itemcais.name
						curentvehicle_model = itemcais.arme
						curentvehicle_modelm = itemcais.money
						curentvehicle_modelc = itemcais.car
						curentvehicle_modelc2 = itemcais.car2
						curentvehicle_point = itemcais.point
						curentvehicle_finalpoint = itemcais.point

						if pointjoueur >= curentvehicle_finalpoint then
							give_vehic(curentvehicle_modelc)
							give_vehic(curentvehicle_modelc2)
							buying(curentvehicle_finalpoint)
							gmoney(curentvehicle_model, curentvehicle_name)
							gmoney(curentvehicle_modelm, curentvehicle_name)
							garme(curentvehicle_model, curentvehicle_name)
						else
							ESX.ShowNotification("~r~Vous n'avez pas assez de fond pour acheter ceci !")
						end

...
RegisterCommand("otage", function(source, args)
    ...
    if not canTakeHostage then
        --drawNativeNotification("Vous devez avoir une arme pour prendre en otage !")
        ESX.ShowNotification("~r~Vous devez être armé.")
    end
    ...
    while true do
        if holdingHostage then
            if GetEntityHealth(GetPlayerPed(-1)) <= 102 then
                --You may need to edit this death check for your server
            ...
            DrawText3D(playerCoords.x, playerCoords.y, playerCoords.z, "Appuyez sur [G] pour libérer, [H] pour tuer")

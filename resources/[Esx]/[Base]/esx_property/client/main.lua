		...
		Citizen.CreateThread(function()
			while aSortir do
				RageUI.IsVisible(RMenu:Get("aSortir",'menu'),true,true,true,function()
					...
					RageUI.ButtonWithStyle("Rendre sa propriété : 0$", nil, {RightLabel = ""},true, function(Hovered, Active, Selected)
						if (Selected) then
					...
					RageUI.ButtonWithStyle("Propriété : " .. property.label, nil, {RightLabel = ""},true, function(Hovered, Active, Selected)
					end)
					if not Config.EnablePlayerManagement then					
						RageUI.ButtonWithStyle("Acheter la propriété : " .. property.price.. "€", nil, {RightLabel = ""},true, function(Hovered, Active, Selected)
						if (Selected) then
							...
							ESX.TriggerServerCallback('property:getPlayerInventory', function(inventory)
								for i=1, #inventory.items, 1 do
									...
									if item.count > 0 then
										table.insert(invItems, {label = '~r~[' .. item.count ..']~r~ ~b~| '.. item.label..''  , type = 'item_standard', value = item.name})
									end
								end                                  
							end)
						end
						...
						if Selected then
							...
							ESX.TriggerServerCallback('property:getStockItems', function(inventory)
								for i=1, #inventory.items, 1 do
									...
									if item.count > 0 then
										table.insert(Items, {label = '~r~[' .. item.count ..']~r~ ~b~| '.. item.label..''  , type = 'item_standard', value = item.name})
									end
								end
								for i=1, #inventory.weapons, 1 do
									local weapon = inventory.weapons[i]				
									table.insert(Itemsss, { label = ESX.GetWeaponLabel(weapon.name) .. ' ~r~[' .. weapon.ammo .. ']~r~ ~b~| ', type  = 'item_weapon', value = weapon.name, index = i })
								end
							end)
						end
				...
				RageUI.IsVisible(RMenu:Get('aProperty', 'enlever'), true, true, true, function()
					...
					for k,v in pairs(Items) do
						RageUI.ButtonWithStyle(v.label  , nil, {RightLabel = count}, true, function(Hovered, Active, Selected)
							if Selected then
								local itemName = v.value
								local result = tonumber(KeyboardInput("~s~Quantité:", "", "", 6))
								if not result then
									ESX.ShowNotification('Quantité invalide')							
								else

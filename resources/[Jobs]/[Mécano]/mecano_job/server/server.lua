...
AddEventHandler('Ouvre:mechanic', function()
	...
	for i=1, #xPlayers, 1 do
		...
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Los Santos Customs', '~p~Annonce', 'Le mecano est désormais ~g~Ouvert~s~!', 'CHAR_LAZLOW', 8)
	end
end)
...
AddEventHandler('Ferme:mechanic', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Los Santos Customs', '~p~Annonce', 'Le mecano est désormais ~r~Fermer~s~!', 'CHAR_LAZLOW', 8)
	end
end)
